...
package com.trekr.Common.HorizontalPicker;
...
public class HorizontalPicker extends View {
    ...
    public HorizontalPicker(Context context, AttributeSet attrs, int defStyle) {
        ...
        CharSequence[] values;
        int ellipsize = 5; // END default value
...
//                textPaint.setTextSize(35);
                ...
                Rect r = new Rect(0, 0, itemWidth, 1000);
    ...
    private int getTextColor(int item) {
        ...
        int scrollX = (int) (getScrollX() + margin);

        // set color of text
        int color = textColor.getDefaultColor();
        if (scrollX > itemWidth * (item + 1) - itemWidth / 2 && scrollX < itemWidth * (item + 2) - itemWidth / 2) {
            color = getColor(scrollX - (itemWidth * item + itemWidth), item - 1);
    ...
    private int getTextSize(int item) {

        int scrollX = (int) (getScrollX() + margin);

        // set color of text
        int size = getDefaultTextSize();
        if (scrollX > itemWidth * (item + 1) - itemWidth / 2 && scrollX < itemWidth * (item + 2) - itemWidth / 2) {
            size = getSize(scrollX - (itemWidth * item + itemWidth), item - 1);
        }
    ...
    private void selectItem() {
        ...
        if (onItemSelected != null) {
            post(new Runnable() {
                @Override
                public void run() {
                    onItemSelected.onDayPickerItemSelected(getSelectedItem(), getSelectedDate());
                }
            });
        }
    ...
    public void setOnTouchedListener(OnTouched onTouched) {
    ...
    public void setDateRange(int x, int y) {

        SimpleDateFormat dateformat = new SimpleDateFormat("EEE dd");

        List<String> listItems = new ArrayList<String>();

        todayIndex = -x / 3600 / 24 + 11;
        beforeDays = x / 3600 / 24;

        String startDate = getDateFromMiliseconds(getCurrentMiliseconds() + TimeUnit.SECONDS.toMillis(x - 10 * 24 * 3600), "yyyy-MM-dd");
        String endDate = getDateFromMiliseconds(getCurrentMiliseconds() + TimeUnit.SECONDS.toMillis(y + 10 * 24 * 3600), "yyyy-MM-dd");
        List<Date> dates = getDates(startDate, endDate);
        for (int i = 0; i < dates.size(); i++) {
            Date d = dates.get(i);
            listItems.add(dateformat.format(d));
        }

        final CharSequence[] charSequenceItems = listItems.toArray(new CharSequence[listItems.size()]);

        setValues(charSequenceItems);
        setSelectedItem(todayIndex);
    }
    ...
    public static String getDateFromMiliseconds(long milliSeconds, String dateFormat)
    {
    ...
    public Date getSelectedDate() {
        ...
        currentCal.add(Calendar.DATE, selectedItem - 11 + beforeDays);
    ...
    private int getPositionFromTouch(float x) {
        if (x > getWidth() / 2 - itemWidth / 2 && x < getWidth() / 2 + itemWidth / 2) {
            return this.selectedItem - 1;
        } else {
            if (x <= getWidth() / 2 - itemWidth / 2) {
                return (int) ((x - getWidth() / 2 + itemWidth / 2) / itemWidth) - 1 + this.selectedItem - 1;
            } else {
                return (int) ((x - getWidth() / 2 - itemWidth / 2) / itemWidth) + 1 + this.selectedItem - 1;
            }
        }
    ...
    private void flingX(int velocityX) {
        ...
        flingScrollerX.fling(getScrollX(), getScrollY(), -velocityX, 0, 0,
                itemWidth * (values.length - 1), 0, 0, getWidth() / 2, 0);

        invalidate();
    }

    public void adjustToNearestItemX() {

        int x = getScrollX();
        int item = Math.round((x + margin) / itemWidth);

        if(item < 10) {
            item = 10;
        } else if(item > values.length - 10) {
            item = values.length - 10;
        }

        selectedItem = item;

        int itemX = (int) (itemWidth * (item) - margin);
...
//        }
        ...
        if (onItemSelected != null) {
            post(new Runnable() {
                @Override
                public void run() {
//                    onItemSelected.onDayPickerItemSelected(getPositionFromCoordinates(getScrollX()), getSelectedDate());
                    onItemSelected.onDayPickerItemSelected(selectedItem, getSelectedDate());
                }
            });
        }
    }
...
//        return (int) (x / itemWidth);
        if (x > getWidth() / 2 - itemWidth / 2 && x < getWidth() / 2 + itemWidth / 2) {
            ...
            if (x <= getWidth() / 2 - itemWidth / 2) {
                return (int) ((x - getWidth() / 2 + itemWidth / 2) / itemWidth) - 1;
            } else {
                return (int) ((x - getWidth() / 2 - itemWidth / 2) / itemWidth) + 1;
            }
        }
    ...
    private int getColor(int scrollX, int position) {
        float proportion = Math.abs(1f * scrollX / itemWidth);
        if(proportion > .5) {
    ...
    private int getSize(int scrollX, int position) {
        float proportion = Math.abs(1f * scrollX / itemWidth);
        if(proportion > .5) {
            proportion = (proportion - .5f);
        } else {
        ...
        return (Integer) new ArgbEvaluator().evaluate(proportion, 45, getDefaultTextSize());
    }
    ...
    private void scrollToItem(int index) {
        margin = (getWidth() - getDefaultItemWidth()) / 2;
        scrollTo((int) (itemWidth * index - margin), 0);
...
//     */
    ...
    private int getInBoundsX(int x) {
        ...
        if(x < 0) {
            x = 0;
        } else if(x > (itemWidth * (values.length - 1))) {
            x = (itemWidth * (values.length - 1));
        }
        return x;
    }
    ...
    private int getScrollRange() {
        ...
        if(values != null && values.length != 0) {
            scrollRange = Math.max(0, (itemWidth * (values.length - 1)));
        }
        return scrollRange;
    }

    public interface OnItemSelected {
        void onDayPickerItemSelected(int index, Date date);
    }

    public interface OnTouched {
        void onDayPickerTouched();
    }
    ...
    private static class PickerTouchHelper extends ExploreByTouchHelper {
        ...
        protected int getVirtualViewAt(float x, float y) {
            ...
            float itemWidth = mPicker.itemWidth;
            float position = mPicker.getScrollX() + x - itemWidth;
        ...
        protected void getVisibleVirtualViews(List<Integer> virtualViewIds) {
            ...
            float itemWidth = mPicker.itemWidth;
            float position = mPicker.getScrollX() - itemWidth;
        ...
        protected void onPopulateNodeForVirtualView(int virtualViewId, AccessibilityNodeInfoCompat node) {
            ...
            float itemWidth = mPicker.itemWidth;
            float scrollOffset = mPicker.getScrollX() - itemWidth;
